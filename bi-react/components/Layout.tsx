...
import { TRANSLATIONS, SAMPLE_INVESTORS, EXTRA_STATS } from '../constants';
import { Logo } from './Logo';
import { Globe, User as UserIcon, LogOut, Home, Briefcase, Users, Bell } from 'lucide-react';
import { Link, useLocation } from 'react-router-dom';

export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const { lang, toggleLanguage, user, logout, projects, unreadNotificationsCount } = useStore();
  ...
  const totalStats = useMemo(() => {
    return projects.length + 
           SAMPLE_INVESTORS.length + 
           EXTRA_STATS.companyOwners + 
           EXTRA_STATS.proposedDeals + 
           EXTRA_STATS.successfulDeals;
  }, [projects]);

  // Mobile Tab Bar Item Component
  const TabItem = ({ to, icon: Icon, label, badge }: { to: string, icon: any, label: string, badge?: number }) => {
    let active = false;
        ...
        <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div>
              ...
              <ul className="space-y-3">
                <li><a href="#" className="text-gray-400 hover:text-white text-sm">{lang === 'en' ? 'Terms of Use' : 'شروط الاستخدام'}</a></li>
                <li><a href="#" className="text-gray-400 hover:text-white text-sm">{lang === 'en' ? 'Privacy Policy' : 'سياسة الخصوصية'}</a></li>
              </ul>
            </div>
          </div>
