    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      {favProjects.length === 0 ? (
        <div className="text-center py-20 bg-brand-gray/30 rounded-xl border border-dashed border-white/10">
          <p className="text-gray-400 mb-4">{lang === 'en' ? 'No favorites yet.' : 'لا توجد مفضلة بعد.'}</p>
          <Link to="/projects" className="text-brand-gold hover:underline">
            {lang === 'en' ? 'Browse Projects' : 'تصفح المشاريع'}
          </Link>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {favProjects.map(project => {
            ...
            return (
              <Link to={`/projects/${project.id}`} key={project.id} className="group bg-brand-gray/30 border border-white/10 hover:border-brand-gold/50 rounded-xl overflow-hidden transition-all hover:-translate-y-1 block relative shadow-lg">
