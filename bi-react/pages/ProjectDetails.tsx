...
import { TRANSLATIONS, FINANCIAL_HEALTH_MAP, COMPANY_STAGES, FALLBACK_IMAGE, AD_STATUS_CONFIG } from '../constants';
...
export const ProjectDetails: React.FC = () => {
  ...
  const { lang, projects, user, unlockProject, isProjectUnlocked, requestInterest, isPubliclyVisible } = useStore();
  ...
  // Check visibility
  const canView = isPubliclyVisible(project) || (user && (user.id === project.ownerId || user.role === 'admin'));
  if (!canView) return <Navigate to="/projects" />;

  const unlocked = isProjectUnlocked(project.id);

  // Get localized financial health
  const healthKey = project.financialHealth;
  const localizedHealth = FINANCIAL_HEALTH_MAP[healthKey] ? FINANCIAL_HEALTH_MAP[healthKey][lang] : healthKey;
      ...
      <div className="relative mb-8 z-20 group">
        ...
        <div className="relative p-8">
          <div className="flex flex-col md:flex-row justify-between items-start mb-8">
            <div>
              <span className={`bg-brand-gold text-black px-3 py-1 rounded text-xs font-bold uppercase tracking-widest ${lang === 'ar' ? 'inline-block mb-4' : ''}`}>
                {project.category[lang]}
              </span>
              <h1 className={`text-3xl md:text-5xl font-bold mb-2 text-white shadow-black drop-shadow-lg ${lang === 'ar' ? 'leading-normal' : 'mt-4'}`}>
                {project.name[lang]}
              </h1>
              <p className="text-gray-300 text-sm flex items-center gap-2 mt-2">
                 ...
                 <span className="flex items-center gap-1 text-brand-gold"><MapPin size={12}/> {project.location[lang]}</span>
              </p>
            </div>
            <div className="mt-4 md:mt-0 flex gap-2">
               {/* Ad Status Badge */}
               <span className={`flex items-center gap-2 text-white text-sm font-bold uppercase tracking-wider ${AD_STATUS_CONFIG[project.status].color} px-4 py-2 rounded border border-white/10`} title={AD_STATUS_CONFIG[project.status].desc[lang]}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-black/60 backdrop-blur-md p-4 rounded-xl border border-white/10 hover:border-brand-gold/30 transition flex flex-col justify-center min-h-[100px] min-w-0">
              <div className="flex items-start gap-2 text-gray-400 mb-1.5 text-xs uppercase w-full">
                ...
                <span className="line-clamp-2 leading-tight">{t.auth.requestedInvestment}</span>
              </div>
            ...
            <div className="bg-black/60 backdrop-blur-md p-4 rounded-xl border border-white/10 hover:border-brand-gold/30 transition flex flex-col justify-center min-h-[100px] min-w-0">
              <div className="flex items-start gap-2 text-gray-400 mb-1.5 text-xs uppercase w-full">
                ...
                <span className="line-clamp-2 leading-tight">{t.common.age}</span>
              </div>
            ...
            <div className="bg-black/60 backdrop-blur-md p-4 rounded-xl border border-white/10 hover:border-brand-gold/30 transition flex flex-col justify-center min-h-[100px] min-w-0">
              <div className="flex items-start gap-2 text-gray-400 mb-1.5 text-xs uppercase w-full">
                ...
                <span className="line-clamp-2 leading-tight">{t.auth.legalEntity}</span>
              </div>
              <div className="text-xs sm:text-sm md:text-lg font-bold text-white whitespace-nowrap overflow-hidden text-ellipsis leading-tight">
                {project.legalEntity ? project.legalEntity[lang] : 'LLC'}
              </div>
            </div>
            ...
            >
               <div className="flex items-start gap-2 text-gray-400 mb-1.5 text-xs uppercase w-full">
                 ...
                 <span className="line-clamp-2 leading-tight">{t.auth.companyStage}</span>
               </div>
               <div className="text-xs sm:text-sm md:text-lg font-bold text-brand-gold whitespace-nowrap overflow-hidden text-ellipsis leading-tight">
                 {project.companyStage ? project.companyStage[lang] : ''}
               </div>
               ...
               {showStageTooltip && stageInfo && (
                 ...
                 >
                    <div className="text-sm text-gray-200 leading-relaxed text-center">
                       <strong className="block text-brand-gold mb-2 border-b border-brand-gold/20 pb-2">{stageInfo.label[lang]}</strong>
                       {stageInfo.desc[lang]}
                    </div>
                    {/* Arrow */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative z-10">
        ...
        <div className="md:col-span-2 space-y-8">
          ...
          <div className="bg-brand-dark p-8 rounded-2xl border border-white/5">
            <h2 className="text-xl font-bold mb-4 border-b border-white/10 pb-4">{lang === 'en' ? 'Project Overview (Summary)' : 'الملف المختصر (نظرة عامة)'}</h2>
            <div className="space-y-6">
              
              <div>
                <h3 className="text-gray-400 text-sm font-bold uppercase mb-2">{t.auth.investmentReason}</h3>
                <p className="text-gray-300 leading-relaxed">
                  {project.investmentReason ? project.investmentReason[lang] : project.descriptionShort[lang]}
                </p>
              </div>
              <div>
                 <h3 className="text-gray-400 text-sm font-bold uppercase mb-2">{lang === 'en' ? 'Activity Description' : 'وصف النشاط'}</h3>
                 <p className="text-gray-300 leading-relaxed">
                   {project.descriptionShort[lang]}
                 </p>
              </div>
            </div>
          </div>
          ...
          {unlocked ? (
            <div className="bg-green-900/10 border border-green-500/20 p-8 rounded-xl animate-fade-in">
              <h2 className="text-xl font-bold mb-4 border-b border-green-500/20 pb-4 text-green-400 flex items-center gap-2">
                <Unlock size={20} /> {lang === 'en' ? 'Project File (Full Details)' : 'كراسة المشروع (التفاصيل الكاملة)'}
              </h2>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                 <div className="bg-black/40 p-4 rounded-lg">
                    <span className="block text-gray-400 text-xs uppercase mb-1">{t.auth.shareToSell}</span>
                    <span className="text-2xl font-bold text-brand-gold font-sans">{project.shareOffered}%</span>
                 </div>
                 <div className="bg-black/40 p-4 rounded-lg">
                    <span className="block text-gray-400 text-xs uppercase mb-1">{t.common.financial}</span>
                    <span className="text-2xl font-bold text-white block mb-1">{localizedHealth}</span>
                    <p className="text-xs text-gray-400 leading-relaxed">
                        {FINANCIAL_HEALTH_MAP[project.financialHealth].desc[lang]}
                    </p>
                 </div>
              </div>
              ...
              <div className="mt-8 flex gap-4 border-t border-green-500/20 pt-6">
                ...
                <a href={`mailto:admin@bi.com?subject=Interest in ${project.id}`} className="flex-1 bg-white/10 text-white font-bold py-3 rounded-lg hover:bg-white/20 transition text-center">
                  {lang === 'en' ? 'Contact Admin' : 'تواصل مع الإدارة'}
                </a>
              </div>
            </div>
          ) : (
            <div className="bg-brand-gray border border-dashed border-white/20 p-10 rounded-xl text-center relative overflow-hidden">
               ...
               <div className="relative z-10">
                ...
                <h3 className="text-lg font-bold text-white mb-2">{lang === 'en' ? 'Full Details & Equity Info Locked' : 'التفاصيل الكاملة وحصة الأسهم مقفلة'}</h3>
                ...
                <button onClick={handlePurchase} className="bg-white text-black px-8 py-3 rounded-lg font-bold hover:bg-gray-200 transition shadow-lg shadow-white/10">
                  {t.common.buyFile} (<Money value={10} />)
                </button>
              </div>
            </div>
          )}
        ...
        <div className="space-y-6">
          <div className="bg-brand-dark p-6 rounded-xl border border-white/5">
             <h3 className="font-bold mb-4 text-gray-200">{lang === 'en' ? 'Investment Highlights' : 'أبرز نقاط الاستثمار'}</h3>
             <ul className="space-y-3">
               <li className="flex items-center gap-3 text-sm text-gray-400">
                 <CheckCircle size={16} className="text-brand-gold" />
                 <span>{lang === 'en' ? 'Verified Ownership' : 'ملكية موثقة'}</span>
               </li>
               <li className="flex items-center gap-3 text-sm text-gray-400">
                 <CheckCircle size={16} className="text-brand-gold" />
                 <span>{lang === 'en' ? 'Audited Financials' : 'بيانات مالية مدققة'}</span>
               </li>
               <li className="flex items-center gap-3 text-sm text-gray-400">
                 <CheckCircle size={16} className="text-brand-gold" />
                 <span>{lang === 'en' ? 'Admin Escrow Available' : 'خدمة الضمان الإداري متاحة'}</span>
               </li>
             </ul>
          </div>
