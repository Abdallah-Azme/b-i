...
import { useMyOpportunities, useDeleteOpportunity } from '../features/company/hooks/useOpportunities';
import { useWhoWeAre } from '../features/general/hooks/useGeneralLookups';
import { useCurrentRequests, usePurchasedSeats } from '../features/company/hooks/useCompanyInteractions';
...
const ProjectCard: React.FC<{ project: any, type: 'ad' | 'booklet' }> = ({ project, type }) => {
  ...
  <div className="relative group block bg-brand-gray/20 rounded-xl overflow-hidden border border-white/5 hover:border-brand-gold/30 transition-all hover:scale-[1.02] shadow-md">
      <Link to="/projects/$id" params={{ id: project.id.toString() }} className="block">
        <div className="aspect-[4/3] bg-gray-800 relative overflow-hidden">
          ...
          {type === 'ad' && AD_STATUS_CONFIG[status]
            ? <div className={`absolute top-2 right-2 px-2 py-1 rounded text-[10px] font-bold text-white flex items-center gap-1 ${AD_STATUS_CONFIG[status].color}`}>
            ...
            : <div className="absolute top-2 right-2 px-2 py-1 rounded text-[10px] font-bold bg-black/60 backdrop-blur text-white flex items-center gap-1">
                <Unlock size={10} className="text-brand-gold" /> {t('dashboard.unlocked')}
              </div>
          }
      ...
      <div className="p-4">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-[10px] text-brand-gold uppercase tracking-wider font-bold">{categoryName}</span>
          </div>
          <h3 className="font-bold text-white mb-3 line-clamp-1 group-hover:text-brand-gold transition text-sm md:text-base">{projectName}</h3>
          ...
          <div className="flex justify-between items-center pt-3 border-t border-white/5">
             ...
             <div className="text-right">
                <span className="block text-[10px] text-gray-500 uppercase">{t('dashboard.value')}</span>
...
export const Dashboard: React.FC = () => {
  ...
  const storedRole = localStorage.getItem('auth_role') as 'investor' | 'advertiser' | null;
  const user = apiUser
    ? {
        ...apiUser,
        role: (apiUser.role as any)?.key ?? apiUser.role ?? storedRole ?? 'investor',
    ...
    return (
      <div className="max-w-md mx-auto px-4 py-8 min-h-[75vh] animate-fade-in flex flex-col">
         ...
         <div className="space-y-3">
            ...
            {/* Contact */}
            <a href={`mailto:${contactEmail}`} className="w-full flex items-center gap-4 p-4 bg-brand-gray/20 border border-white/5 rounded-xl hover:bg-brand-gray/40 transition">
  ...
  const { data: myOpsData, isLoading: opsLoading } = useMyOpportunities({ per_page: 50, page: adsPage }, { enabled: user.role === 'advertiser' });
  const myAds = myOpsData?.data?.opportunities ?? [];
  const { data: companySeatsData } = usePurchasedSeats({}, { enabled: user.role === 'advertiser' });
  const { data: investorSeatsData } = useInvestorPurchasedSeats({}, { enabled: user.role !== 'advertiser' });
  
  const myBooklets = investorSeatsData?.data?.seats ?? [];
  const successfulDeals = companySeatsData?.data?.seats?.length || 0; 
  const totalValue = myAds.reduce((acc: number, curr: any) => acc + (curr.investment_required || 0), 0);
  ...
  const stats = user.role === 'advertiser' 
    ? [
        { icon: Megaphone, value: myAds.length, label: t('dashboard.myAds'), type: 'number' },
        { icon: CheckCircle, value: successfulDeals, label: t('dashboard.successfulDeals'), type: 'number' },
        { icon: DollarSign, value: totalValue, label: t('dashboard.portfolioValue'), type: 'compact' }
      ]
    : [
        { icon: FileText, value: myBooklets.length, label: t('dashboard.booklets'), type: 'number' },
        { icon: TrendingUp, value: successfulInvestments, label: t('dashboard.successfulInvestments'), type: 'number' },
        { icon: Wallet, value: capital, label: t('dashboard.capital'), type: 'compact' }
      ];
  ...
  const tabs = user.role === 'advertiser' 
    ? [
        ...
        { id: 'booklets', label: t('dashboard.purchasedBooklets'), icon: FileText },
        { id: 'incoming', label: t('dashboard.incomingRequests'), icon: Mail },
        { id: 'verification', label: t('dashboard.verificationInfo'), icon: Shield },
        { id: 'settings', label: t('dashboard.settings'), icon: Settings },
      ]
    : [
        { id: 'booklets', label: t('dashboard.purchasedBooklets'), icon: FileText },
        { id: 'interests', label: t('dashboard.sentInterests'), icon: Star },
        { id: 'requests', label: t('dashboard.ongoingRequests'), icon: Briefcase },
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 md:py-12 min-h-screen relative">
      ...
      <div className="animate-fade-in min-h-[300px]">
         {activeTab === 'ads' && (
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                 {myAds.length > 0 ? myAds.map((p: any) => (
              ...
              {/* Pagination for Ads */}
              {myOpsData?.data?.pagination && myOpsData.data.pagination.last_page > 1 && (
         ...
         {activeTab === 'booklets' && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
               {myBooklets.length > 0 ? myBooklets.map((seat: any) => (
                 <ProjectCard key={seat.id} project={seat.opportunity} type="booklet" />
               )) : (
                 <EmptyState 
                   icon={FileText} 
         ...
         {activeTab === 'verification' && <VerificationInfoTab onEditProfile={() => setIsEditProfileOpen(true)} />}
