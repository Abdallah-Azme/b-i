            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                >
                  ...
                  <option value="request_investment">{t('filters.forInvestment')}</option>
                </select>
              </div>

              <button 
                onClick={() => { setFilterCat(undefined); setFilterPurpose(undefined); }}
            ...
            {isLoading ? (
              ...
              <div className="grid grid-cols-1 gap-6">
                {opportunities.map(project => {
                  ...
                  return (
                    <div key={project.id} className="glass-card hover:border-brand-gold/50 rounded-xl overflow-hidden transition-all hover-scale group relative">
                        ...
                        <div className="flex-1 p-6 flex flex-col justify-between">
                          <div className="relative">
                             ...
                             <div className="flex items-center gap-2 mb-1">
                                <span className="text-brand-gold text-xs font-bold uppercase tracking-widest block">{project.category.name}</span>
                             ...
                             <h3 className="text-xl font-bold mb-2 group-hover:text-white text-gray-100">{project.company_name}</h3>
                             
                             <div className="flex flex-wrap gap-4 text-sm text-gray-400 mb-4">
                               <span className="bg-black/20 px-3 py-1 rounded-full border border-white/5">
                                 {t('common.price')}: <Money value={project.investment_required} className="text-white font-sans" />
                               </span>
                               {project.sale_percentage && (
                                 <span className="bg-black/20 px-3 py-1 rounded-full border border-white/5">
                                   {t('common.share')}: <span className="text-white font-sans">{project.sale_percentage}%</span>
                                 </span>
                               )}
                             </div>
                          </div>
