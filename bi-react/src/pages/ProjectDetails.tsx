...
import { useBuySeat, useSubmitInterestRequest } from '../features/general/hooks/useActions';
    ...
    buySeat.mutate({ id: project.id }, {
      onSuccess: (response: any) => {
        ...
        if (sid) {
          ...
          queryClient.invalidateQueries({ queryKey: ['opportunity-detail', project.id] });
        }
      }
    });
  };
        ...
        <div className="relative p-8">
          <div className="flex flex-col md:flex-row justify-between items-start mb-8">
            <div>
              ...
              <p className="text-gray-300 text-sm flex items-center gap-2 mt-2">
                 <span className="opacity-60 font-sans">ID: {project.opportunity_number}</span>
              </p>
            </div>
            <div className="mt-4 md:mt-0 flex gap-2">
               {/* Ad Status Badge */}
               <span className={`flex items-center gap-2 text-white text-sm font-bold uppercase tracking-wider bg-black/60 px-4 py-2 rounded border border-white/10`} title={project.status.label}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-black/60 backdrop-blur-md p-4 rounded-xl border border-white/10 hover:border-brand-gold/30 transition flex flex-col justify-center min-h-[100px] min-w-0">
              ...
              <Money value={project.investment_required} className="text-xs sm:text-sm md:text-lg font-bold text-white whitespace-nowrap overflow-hidden text-ellipsis leading-tight font-sans" />
            </div>
            ...
            <div className="bg-black/60 backdrop-blur-md p-4 rounded-xl border border-white/10 hover:border-brand-gold/30 transition flex flex-col justify-center min-h-[100px] min-w-0">
              <div className="flex items-start gap-2 text-gray-400 mb-1.5 text-xs uppercase w-full">
                ...
                <span className="line-clamp-2 leading-tight">{t('auth.companyTypeLabel')}</span>
              </div>
               ...
               {showStageTooltip && project.business_stage && (
                 ...
                 >
                    <div className="text-sm text-gray-200 leading-relaxed text-center">
                       <strong className="block text-brand-gold mb-2 border-b border-brand-gold/20 pb-2">{project.business_stage}</strong>
                    </div>
                    {/* Arrow */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative z-10">
        ...
        <div className="md:col-span-2 space-y-8">
          ...
          <div className="bg-brand-dark p-8 rounded-2xl border border-white/5">
            <h2 className="text-xl font-bold mb-4 border-b border-white/10 pb-4">{t('common.overview')}</h2>
          ...
          {unlocked ? (
            <div className="bg-green-900/10 border border-green-500/20 p-8 rounded-xl animate-fade-in">
              <h2 className="text-xl font-bold mb-4 border-b border-green-500/20 pb-4 text-green-400 flex items-center gap-2">
                <Unlock size={20} /> {t('common.projectFile')}
                 ...
                 <div className="bg-black/40 p-4 rounded-lg">
                    ...
                    <span className="text-2xl font-bold text-brand-gold font-sans">{project.sale_percentage ?? 0}%</span>
                 </div>
                 <div className="bg-black/40 p-4 rounded-lg">
                    <span className="block text-gray-400 text-xs uppercase mb-1">{t('common.financial')}</span>
                    <span className="text-2xl font-bold text-white block mb-1">{project.financial_status}</span>
                 </div>
              </div>
              ...
              <div 
                ...
                dangerouslySetInnerHTML={{ __html: project.full_description || t('common.noDetails') }}
              />
              
              <div className="mt-8 flex gap-4 border-t border-green-500/20 pt-6">
                <button 
                  onClick={() => {
                    if (!user) { navigate({ to: '/login-type' }); return; }
                    submitInterest.mutate({ id: project.id });
                  }} 
                  disabled={submitInterest.isPending || submitInterest.isSuccess}
                ...
                >
                  {submitInterest.isPending ? <Loader2 className="animate-spin" size={20} /> : (submitInterest.isSuccess ? 'Interest Submitted ✓' : t('common.interested'))}
                </button>
                <a href={`mailto:${contactEmail}?subject=Interest in ${project.id}`} className="flex-1 flex justify-center items-center bg-white/10 text-white font-bold py-3 rounded-lg hover:bg-white/20 transition text-center">
            ...
            <div className="bg-brand-gray border border-dashed border-white/20 p-10 rounded-xl text-center relative overflow-hidden">
               ...
               <div className="relative z-10">
                ...
                <h3 className="text-lg font-bold text-white mb-2">{t('common.lockedTitle')}</h3>
        ...
        <div className="space-y-6">
          <div className="bg-brand-dark p-6 rounded-xl border border-white/5">
             <h3 className="font-bold mb-4 text-gray-200">{t('common.highlights')}</h3>
